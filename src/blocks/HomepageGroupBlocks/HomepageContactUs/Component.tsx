...
export const HomepageContactUsBlock: React.FC<HomepageContactUsBlockType> = ({
  ...
  const logoAlt =
    typeof logo === "object" && logo !== null ? (logo as Media).alt || companyName : companyName;

  const getSocialIcon = (platform: string) => {
    switch (platform) {
      case "facebook":
        return <Facebook size={18} fill="currentColor" strokeWidth={0} className="text-white" />;
      case "twitter":
        return <Twitter size={18} fill="currentColor" strokeWidth={0} className="text-white" />;
      case "linkedin":
        return <Linkedin size={18} fill="currentColor" strokeWidth={0} className="text-white" />;
      case "instagram":
  ...
  return (
    <section
      className={cn("py-16 md:py-20", getBgClass())}
      style={{ backgroundColor: backgroundColor === "lightGray" ? "#eaeaea" : "#ffffff" }}
    >
          ...
          <div className="flex flex-col md:flex-row flex-wrap items-center justify-center gap-y-4 gap-x-6 md:gap-x-8 lg:gap-x-12 mb-8 text-[14px] md:text-[15px] text-[#4b5563]">
            ...
            {email && (
              <div className="flex items-center gap-2">
                ...
                <a href={`mailto:${email}`} className="hover:text-[#1e3a5f] transition-colors">
                  {email}
                </a>
              </div>
            )}
